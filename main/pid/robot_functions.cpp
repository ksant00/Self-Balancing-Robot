...
/**
 * Interrupt Handler for right sonar echo pin
 * 
...
/**
 * Interrupt Handler for left sonar echo pin
 * 
 * Interrupt event happens every digital state change of echo pin
 * Takes the start and end time for echo signal
 */
...
void pid_controller::update (void) {
    ...
    if ((app.stop != 0) && app.movement == 0) {
        ...
        integral = constrain(integral, -500, 500); // contrains the error to prevent integral windup
    }
...
void update_pwm (void) {
    if (pid.control_sig < 0) {
        if(pid.control_sig != 0) {
                if (app.backwards && app.movement == 2) {
                    pwm_values[1] = (1 << 15) | map(abs(pid.control_sig * 1.3), 0, 1000, 965, 0);
                    pwm_values[3] = (1 << 15) | map(abs(pid.control_sig * 0.6), 0, 1000, 965, 0);
                } else if (app.backwards && app.movement == 3) {
                    pwm_values[1] = (1 << 15) | map(abs(pid.control_sig * 0.6), 0, 1000, 965, 0);
                    pwm_values[3] = (1 << 15) | map(abs(pid.control_sig * 1.3), 0, 1000, 965, 0);
                } else {
                    pwm_values[1] = (1 << 15) | map(abs(pid.control_sig), 0, 1000, 962, 0);
                    pwm_values[3] = (1 << 15) | map(abs(pid.control_sig), 0, 1000, 962, 0);
                }
        } else {
        ...
        if(pid.control_sig != 0) {
            if (app.movement == 2 && !app.backwards) { // turns left with motors spining at opposite direction at moving angle
                pwm_values[0] = (1 << 15) | map(abs(pid.control_sig * 1.3), 0, 1000, 1000, 0);
                pwm_values[2] = (1 << 15) | map(abs(pid.control_sig * 0.6), 0, 1000, 1000, 0);

                
                pwm_values[1] = (1 << 15) | 1000;
                pwm_values[3] = (1 << 15) | 1000;
            } else if (app.movement == 3 && !app.backwards) { // turns right with motors spinning at scaled output at 0 degrees
                pwm_values[0] = (1 << 15) | map(abs(pid.control_sig * 0.6), 0, 1000, 1000, 0);
                pwm_values[2] = (1 << 15) | map(abs(pid.control_sig * 1.3), 0, 1000, 1000, 0);
...
/***
 * Function to initialize sonar
 * 
 * Sets trigger pins to outputs and echo pints to inputs
 * Sets an interrupt for echo pins for change in digital logic
 */
...
/** 
 * Function to calculate the distance detected from each sonar
 */
void sonar::get_distance (void) {
    ...
    delay(2);
    sonar1_distance = (sonar1_end_time - sonar1_start_time)*0.0343/2.0;
    sonar2_distance = (sonar2_end_time - sonar2_start_time)*0.0343/2.0;
}
...
/***
 * Function to update the movement controls of robot
 * 
 * Sets the setpoint based on direction
 * Includes adaptive braking logic based on speed
 */
...
/**
 * Function to use extra features
 */
void app_controls::update_extra (int temp) {
    switch (temp) {
        case 5: // Autopilot
            autopilot = !autopilot;
            if (autopilot) {
...
/** 
 * Function to update autopilot mode
 * 
 * If right sonar detects wall, turn backwards right then forward
 * If left sonar detects wall, turn backwards left then forward
 */
void app_controls::update_autopilot(void) {
    if (sonars.sonar1_distance < 30 && movement == 1) {
        movement = 3;
        backwards = 1;
    } else if (sonars.sonar2_distance < 30 && movement == 1) {
        movement = 2;
        backwards = 1;
    }
    
    if ((movement == 2 || movement == 3) && backwards && autopilot_count < 20 && comp_fil.theta > 0) {
        autopilot_count++;
    } else if ((movement == 2 || movement == 3) && backwards && autopilot_count >= 20) {
        autopilot_count = 0;
        movement = 1;
        backwards = 0;
    }
...
/**
 * Function for adaptive braking
 * 
 * Increases count based on loop time for how long bot is moving in certain direction
 */
void app_controls::update_stop (void) {
    if (movement != 0 && backwards) {
        stop_count--;
        stop_count = constrain(stop_count, -22, 22); // Constrain the count to not make angle too agressive or go to infinity
    } else if (movement != 0 && !backwards) {
        stop_count++;
        stop_count = constrain(stop_count, -22, 22); // Constrain the count to not make angle too agressive or go to infinity
    } else if (movement == 0 && stop != 0 && backwards) { // backward direction stop
        if (comp_fil.theta <= 0) {
            stop_count++;
            if (stop_count >= 0) {
                pid.setpoint = 0;
    ...
    } else if (movement == 0 && stop != 0 && !backwards) { // forward direction stop 
        if (comp_fil.theta >= 0) {
            stop_count--;
            if (stop_count <= 0) {
                pid.setpoint = 0;
